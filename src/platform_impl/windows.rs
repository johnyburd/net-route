...
use winapi::shared::netioapi::{CreateIpForwardEntry2, NotifyRouteChange2, MIB_NOTIFICATION_TYPE, CancelMibChangeNotify2, MibParameterNotification, MibAddInstance, MibDeleteInstance, PMIB_IPFORWARD_TABLE2, GetIpForwardTable2, FreeMibTable};
...
unsafe fn row_to_route(row: *mut MIB_IPFORWARD_ROW2) -> Option<Route> {
    ...
    let gateway = match *nexthop_family as i32 {
        AF_INET => Some(IpAddr::from(std::mem::transmute::<_, [u8; 4]>((*row).DestinationPrefix.Prefix.Ipv4().sin_addr))),
        AF_INET6 => Some(IpAddr::from(std::mem::transmute::<_, [u8; 16]>((*row).DestinationPrefix.Prefix.Ipv6().sin6_addr))),
        _ => None
    };
...
impl Handle {
    pub fn new() -> io::Result<Self> {
        ...
        let ret = unsafe { NotifyRouteChange2(AF_UNSPEC as u16, Some(callback), (tx_clone.as_mut() as *mut _) as *mut _, BOOLEAN::from(false), &mut handle) };
    ...
    pub(crate) async fn list(&self) -> io::Result<Vec<Route>> {
        ...
        let prows = unsafe { std::ptr::slice_from_raw_parts(
